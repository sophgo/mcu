...
static struct filter adc_averge_tab[2];
...
void mcu_process(void)
{
	...
	if (current_time - last_time_collect > COLLECT_INTERVAL) {
		for (i = 0; i < 2; ++i) {
			adc_date = adc_read((unsigned long)(i + 8));
...
{
	int V_5V = adc_averge_tab[0].value * 3300 / 4096;
	int V_VDD_12V = adc_averge_tab[1].value * 3300 / 4096;
	printf("V_5V = %d(mV)\n", V_5V);
