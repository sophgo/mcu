...
static void system_gpio_init(void)
{
	...
	pins = GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11 |
	...
	/* GPIOB ANALOG */
	gpio_init(GPIOB, GPIO_MODE_AIN, GPIO_OSPEED_2MHZ, GPIO_PIN_0 | GPIO_PIN_1);

	/* GPIOC */
	/* GPIOC OUTPUT PINS*/
	pins = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_7 | GPIO_PIN_8;
	gpio_bit_reset(GPIOC, pins);
	gpio_init(GPIOC, GPIO_MODE_OUT_PP, GPIO_OSPEED_2MHZ, pins);

	/* GPIOC INPUT PINS */
	pins = GPIO_PIN_6 | GPIO_PIN_13 | GPIO_PIN_14 | GPIO_PIN_15;
	gpio_init(GPIOC, GPIO_MODE_IPU, GPIO_OSPEED_2MHZ, pins);

	/* ANALOG */
	pins = GPIO_PIN_9;
	gpio_init(GPIOC, GPIO_MODE_AIN, GPIO_OSPEED_2MHZ, pins);

	/* GPIOD */
	/* GPIOD OUTPUT PINS*/
	pins = GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14;
	gpio_bit_reset(GPIOD, pins);
	gpio_init(GPIOD, GPIO_MODE_OUT_PP, GPIO_OSPEED_2MHZ, pins);

	/* GPIOD INPUT PINS */
	pins = GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_15;
	gpio_bit_reset(GPIOD, pins);
	gpio_init(GPIOD, GPIO_MODE_IPU, GPIO_OSPEED_2MHZ, pins);

	/* GPIOE */
	/* GPIOE OUTPUT PINS*/
	pins = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_2 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 |
		 GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_14;
	gpio_bit_reset(GPIOE, pins);
	gpio_init(GPIOE, GPIO_MODE_OUT_PP, GPIO_OSPEED_2MHZ, pins);

	/* GPIOE INPUT PINS*/
	pins = GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_10;
...
void system_i2c2_init(void)
{
	...
	gpio_afio_port_config(AFIO_PB5_I2C2_CFG, ENABLE);

	gpio_init(MCU_I2C2_SCL_PORT, GPIO_MODE_AF_OD, GPIO_OSPEED_10MHZ, MCU_I2C2_SCL_PIN);
	gpio_init(MCU_I2C2_SDA_PORT, GPIO_MODE_AF_OD, GPIO_OSPEED_10MHZ, MCU_I2C2_SDA_PIN);
