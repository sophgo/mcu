...
#define REG_PWM_VOLT		0x2a
#define REG_VDDQ_VOLT		0x2b
...
#define CMD_SET_TPU_VOLT	0x0b
#define CMD_SET_VDDQLP_VOLT	0x0c
#define CMD_SET_VDDQ_VOLT	0x0d
#define CMD_SET_BUCK0_VOLT	0x0e
...
void mcu_process(void)
{
	...
	case CMD_SET_VDDC_VOLT:
		debug("set pwm value to %ld for vddc\n", mcu_ctx.pwm_value);
		pwm_set_duty_cycle(mcu_ctx.pwm_value);
		break;
	case CMD_SET_TPU_VOLT:
		debug("set pwm value to %ld for tpu\n", mcu_ctx.pwm_value);
		pwm_set_t2_duty_cycle(mcu_ctx.pwm_value);
		break;
	case CMD_SET_VDDQLP_VOLT:
		debug("set VDDQLP value to %ld\n", mcu_ctx.vddq_value);
		mp5475_voltage_config(1,mcu_ctx.vddq_value);
		break;
	case CMD_SET_VDDQ_VOLT:
		debug("set VDDQ value to %ld\n", mcu_ctx.vddq_value);
		mp5475_voltage_config(2,mcu_ctx.vddq_value);
		break;
	case CMD_SET_BUCK0_VOLT:
		debug("set BUCK0 value to %ld\n", mcu_ctx.vddq_value);
		mp5475_voltage_config(0,mcu_ctx.vddq_value);
		break;
	default:
...
static void mcu_write(void *priv, volatile uint8_t data)
{
	...
	case REG_PWM_VOLT:
		...
		ctx->pwm_value = data*10;
		break;
	case REG_VDDQ_VOLT:
		debug("Get vddq data: %d\n",data);
		ctx->vddq_value = data;
		break;
	default:
