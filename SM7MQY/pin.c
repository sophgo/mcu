...
void pin_init(void)
{
	...
	GPIO_OTYPER(GPIOA) = (GPIO_OTYPE_OD << 8);
	/* output speed, low, medium, high, very high */
	GPIO_OSPEEDR(GPIOA) =
	    (GPIO_OSPEEDR(GPIOA) & ~0xc3ff0000) | (GPIO_OSPEED_HIGH << (8 * 2))
	...
	GPIO_PUPDR(GPIOA) =
	    ...
	    (GPIO_PUPD_NONE << (8 * 2)) | (GPIO_PUPD_PULLDOWN << (9 * 2)) |
	...
	/* AFRL reset state */
	GPIO_AFRH(GPIOA) =
	    (GPIO_AFRH(GPIOA) & ~0x0000000f) | (GPIO_AF7 << ((8 - 8) * 4));
	...
	GPIO_MODER(GPIOA) =
	    ...
	    (GPIO_MODE_INPUT << (7 * 2)) | (GPIO_MODE_AF << (8 * 2)) |
	...
	/* output type, push-pull or open-drain */
	GPIO_OTYPER(GPIOB) =
	    (GPIO_OTYPE_OD << 4) | (GPIO_OTYPE_OD << 6) | (GPIO_OTYPE_OD << 7) |
	    (GPIO_OTYPE_OD << 10) | (GPIO_OTYPE_OD << 11);
	/* output speed, low, medium, high, very high */
	GPIO_OSPEEDR(GPIOB) =
	    (GPIO_OSPEEDR(GPIOB) & ~0x3cfcfff3) | (GPIO_OSPEED_LOW << (0 * 2)) |
	    (GPIO_OSPEED_LOW << (2 * 2)) | (GPIO_OSPEED_LOW << (3 * 2)) |
	    (GPIO_OSPEED_HIGH << (4 * 2)) | (GPIO_OSPEED_LOW << (5 * 2)) |
	    (GPIO_OSPEED_HIGH << (6 * 2)) | (GPIO_OSPEED_HIGH << (7 * 2)) |
	    (GPIO_OSPEED_LOW << (9 * 2)) | (GPIO_OSPEED_HIGH << (10 * 2)) |
	    (GPIO_OSPEED_HIGH << (11 * 2)) | (GPIO_OSPEED_LOW << (13 * 2)) |
	    (GPIO_OSPEED_LOW << (14 * 2));
	/* pull-up pull-down */
	GPIO_PUPDR(GPIOB) =
	    (GPIO_PUPDR(GPIOB) & ~0xfffcfff3) | (GPIO_PUPD_PULLDOWN << (0 * 2))
	    | (GPIO_PUPD_PULLDOWN << (2 * 2)) | (GPIO_PUPD_PULLDOWN << (3 * 2))
	    | (GPIO_PUPD_NONE << (4 * 2)) | (GPIO_PUPD_PULLDOWN << (5 * 2)) |
	    (GPIO_PUPD_NONE << (6 * 2)) | (GPIO_PUPD_NONE << (7 * 2)) |
	    (GPIO_PUPD_PULLDOWN << (9 * 2)) | (GPIO_PUPD_NONE << (10 * 2)) |
	    (GPIO_PUPD_NONE << (11 * 2)) | (GPIO_PUPD_NONE << (12 * 2)) |
	    (GPIO_PUPD_PULLUP << (13 * 2)) | (GPIO_PUPD_PULLDOWN << (14 * 2)) |
	    (GPIO_PUPD_NONE << (15 * 2));
	/* af, alternative function selection */
	GPIO_AFRL(GPIOB) =
	    (GPIO_AFRL(GPIOB) & ~0xff0f0000) | (GPIO_AF7 << (4 * 4)) | (GPIO_AF1
									<< (6 *
									    4))
	    | (GPIO_AF1 << (7 * 4));
	GPIO_AFRH(GPIOB) =
	...
	/* mode, input, output, alternate function or analog */
	GPIO_MODER(GPIOB) =
	    (GPIO_MODER(GPIOB) & ~0xfffcfff3) | (GPIO_MODE_OUTPUT << (0 * 2)) |
	    (GPIO_MODE_OUTPUT << (2 * 2)) | (GPIO_MODE_OUTPUT << (3 * 2)) |
	    (GPIO_MODE_AF << (4 * 2)) | (GPIO_MODE_OUTPUT << (5 * 2)) |
	    (GPIO_MODE_AF << (6 * 2)) | (GPIO_MODE_AF << (7 * 2)) |
