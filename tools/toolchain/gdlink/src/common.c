...
#define GD32E50X_FMC_BASE	0x40022000
#define GD32E50X_FMC_WS		0x00
#define GD32E50X_FMC_KEY	0x04
#define GD32E50X_FMC_OBKEY	0x08
#define GD32E50X_FMC_STAT	0x0c
#define GD32E50X_FMC_CTL	0x10
#define GD32E50X_FMC_ADDR	0x14
#define GD32E50X_FMC_OBSTAT	0x1c
#define GD32E50X_FMC_WP		0x20
#define GD32E50X_FMC_PID	0x100

#define GD32E50X_RCU_BASE	0x40021000
#define GD32E50X_RCU_AHBEN	0x14

#define GD32E50X_REG_ADDR(mod, reg)	\
	(GD32E50X_ ## mod ## _BASE + GD32E50X_ ## mod ## _ ## reg)

static void wait_flash_busy(stlink_t *sl);

static inline int gd32e50x_flash_is_busy(stlink_t *sl)
{
	int err;
	uint32_t stat;

	err = stlink_read_debug32(sl, GD32E50X_REG_ADDR(FMC, STAT), &stat);
	if (err) {
		ELOG("read FMC_STAT failed\n");
		return true;
	}
...
static inline void gd32e50x_flash_clear_error(stlink_t *sl)
{
	int err;

	/* ENDF, WPERR, PGAERR, PGERR */
	err = stlink_write_debug32(sl, GD32E50X_REG_ADDR(FMC, STAT),
				   (1 << 2) | (1 << 3) | (1 << 4));
	if (err) {
		ELOG("write FMC_STAT failed\n");
		return;
	}
}

static inline int gd32e50x_flash_check_error(stlink_t *sl)
{
	int err;
	uint32_t stat;
	const uint32_t err_mask = (1 << 2) | (1 << 3) | (1 << 4);
	...
	/* ENDF, end of operation flag, it is not an error, just identify an
	 * operation done, clear it by writing 1 to this bit */
	if (stat & (1 << 5))
		stlink_write_debug32(sl, GD32E50X_REG_ADDR(FMC, STAT),
				     (stat & ~err_mask));

	stat &= err_mask;

	if (stat)
		ELOG("error occur on FMC, 0x%08x\n", stat);

	return stat;
}

static inline void gd32e50x_flash_erase_page(stlink_t *sl, uint32_t addr)
{
	uint32_t ctl;
	int err;

	err = stlink_read_debug32(sl, GD32E50X_REG_ADDR(FMC, CTL), &ctl);
	if (err) {
		ELOG("read FMC_CTL failed\n");
		return;
	}

	/* PER bit */
	ctl |= 1 << 1;
	err = stlink_write_debug32(sl, GD32E50X_REG_ADDR(FMC, CTL), ctl);
	if (err) {
		ELOG("write FMC_CTL failed\n");
		return;
	}

	err = stlink_write_debug32(sl, GD32E50X_REG_ADDR(FMC, ADDR), addr);
	if (err) {
		ELOG("write FMC_ADDR failed\n");
		return;
	}
...
static inline void gd32e50x_flash_set_pg(stlink_t *sl)
{
	int err;
	uint32_t ctl;
...
static inline void gd32e50x_flash_clear_pg(stlink_t *sl)
{
	int err;
	uint32_t ctl;
...
static inline void gd32e50x_flash_program_word(stlink_t *sl,
					       uint32_t addr, uint32_t data)
{
	int err;

	wait_flash_busy(sl);
	gd32e50x_flash_set_pg(sl);
	err = stlink_write_debug32(sl, addr, data);
	if (err) {
		gd32e50x_flash_clear_pg(sl);
		ELOG("program main flash 0x%08x failed\r", addr);
		return;
	}
	wait_flash_busy(sl);
	gd32e50x_flash_clear_pg(sl);
...
static inline int gd32e50x_flash_check_protection(stlink_t *sl)
{
	/* add some debug information */
	uint32_t fmc_wp;
	stlink_read_debug32(sl, GD32E50X_REG_ADDR(FMC, WP), &fmc_wp);

	int i;

	for (i = 0; i < 32; ++i) {
		if (((fmc_wp >> i) & 1) == 0) {
			WLOG("page %d is protected by optional bytes\n", i);
			WLOG("page %d address %08lx\n", i, i * sl->flash_pgsz);
			return -1;
		}
	}
	return 0;
...
static inline unsigned int is_flash_locked(stlink_t *sl) {
  ...
  } else if (sl->flash_type == STLINK_FLASH_TYPE_GD32E50X) {
    cr_reg = GD32E50X_REG_ADDR(FMC, CTL);
    cr_lock_shift = 7;
  } else {
...
static void unlock_flash(stlink_t *sl) {
  ...
  if (sl->flash_type == STLINK_FLASH_TYPE_F0) {
    ...
    if (sl->chip_flags & CHIP_F_HAS_DUAL_BANK) {
	  ...
	  key_reg = GD32E50X_REG_ADDR(FMC, KEY);
...
static inline unsigned int is_flash_busy(stlink_t *sl) {
  ...
  if ((sl->flash_type == STLINK_FLASH_TYPE_F0) ||
    ...
    sr_busy_shift = FLASH_H7_SR_QW;
	  ...
	  return gd32e50x_flash_is_busy(sl);
  }
  else {
...
static void clear_flash_error(stlink_t *sl) {
  ...
  case STLINK_FLASH_TYPE_WB:
    ...
    break;
  case STLINK_FLASH_TYPE_GD32E50X:
    gd32e50x_flash_clear_error(sl);
    break;
  default:
    break;
  }
...
static int check_flash_error(stlink_t *sl) {
  ...
  default:
  case STLINK_FLASH_TYPE_GD32E50X:
    return gd32e50x_flash_check_error(sl);
    break;
  }
...
static void set_dma_state(stlink_t *sl, flash_loader_t *fl, int bckpRstr) {
  ...
  case STLINK_FLASH_TYPE_WB:
    ...
    break;
  case STLINK_FLASH_TYPE_GD32E50X:
    rcc = GD32E50X_REG_ADDR(RCU, AHBEN);
    rcc_dma_mask = 0x00000003;
    break;
  default:
    return;
  }
...
int stlink_chip_id(stlink_t *sl, uint32_t *chip_id) {
  ...
  if ((sl->core_id == STM32H7_CORE_ID || sl->core_id == STM32H7_CORE_ID_JTAG) &&
    ...
    printf("ARM cortex-m33 detected, chip ID %08x\n", *chip_id);
  } else /* СM3, СM4, CM7 */ {
  ...
  } else if (*chip_id == STLINK_CHIPID_GD32_E50X) {
	  printf("GD32E50x chip id detected\n");
  } else {
...
int stlink_load_device_params(stlink_t *sl) {
  ...
  if (sl->chip_id == STLINK_CHIPID_GD32_E50X) {
  ...
  if (sl->chip_id == STLINK_CHIPID_GD32_E50X) {
	  /* correct sram size */
	  uint32_t memory_density;
	  stlink_read_debug32(sl, (params->flash_size_reg) & ~3,
			      &memory_density);
	  /* unit is KB */
	  sl->sram_size = (memory_density >> 16) * 1024;
  }
...
int stlink_reset(stlink_t *sl, enum reset_type type) {
  ...
  if (type == RESET_HARD || type == RESET_AUTO) {
    ...
    if (sl->chip_id == STLINK_CHIPID_GD32_E50X)
	    printf("skip backend reset on GD32E50x\n");
    else if (sl->backend->reset(sl))
		    return (-1);
    usleep(10000);
  }
...
int stlink_erase_flash_page(stlink_t *sl, stm32_addr_t flashaddr) {
  ...
  if (sl->flash_type == STLINK_FLASH_TYPE_F4 ||
    ...
    lock_flash(sl);
	  ...
	  unlock_flash_if(sl);
	  gd32e50x_flash_erase_page(sl, flashaddr);
	  lock_flash(sl);
  } else {
    ...
    if (sl->verbose >= 1) {
	  ...
	  for (off = 0; off < len; off += 4) {
		  write_uint32((unsigned char *)&data,
			       *(uint32_t *)(base + off));
		  gd32e50x_flash_program_word(sl, addr + off, data);
		  printf("[%08x]  ---->  [%08x] : %ld%%\r", addr, data,
			 off * 100 / len);
	  }
...
int stlink_write_flash(stlink_t *sl, stm32_addr_t addr, uint8_t *base,
  ...
  /* add some debug information */
  if (sl->chip_id == STLINK_CHIPID_GD32_E50X)
	  if (gd32e50x_flash_check_protection(sl))
		  return -1;
