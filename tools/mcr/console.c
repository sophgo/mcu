...
int console_open(struct console_ctx *ctx, char *uart_port)
{
	...
	err = uart_open(&ctx->uart, uart_port);
	if (err)
		return err;
...
static int type_in_command(struct console_ctx *ctx, char *cmd)
{
	...
	for (p = cmd; *p; ++p) {
		if (send_single(ctx, *p, 1000)) {
			error("error with command %s when sending %c\n",
			      cmd, *p);
			return -1;
		}
	}
		 ...
		 char *output, int outlen, int timeout)
{
	...
	if (err) {
		error("cannot reset mcu console\n");
		return -1;
	}
	...
	for (i = 0; i < outlen - 1;) {
		...
		if (timer_remain(&timer) == 0) {
			error("command %s not done in %d ms\n",
				cmd, timeout);
			err = -1;
			break;
		}
	...
	if (i == outlen - 1) {
		error("no enough output buffer for command %s\n",
			cmd);
		return -1;
	}
