...
int xmodem_send(struct uart_ctx *uart, void *data, long len)
{
	...
	if (len % 128) {
		error("only 128 bytes aligned data is supported\n");
		return -1;
	}

	/* wait start condition 'C' */
	if (!uart_wait(uart, CRC, 5000)) {
		error("no start condition found\n");
		return -1;
	}
	...
	for (i = 0; i < total; ++i) {
		...
		while (!send_package(uart, ((char *)data) + i * 128, 128, i)) {
			error("%ldth package send failed, try again\n", i);
			if (timer_remain(&timer) == 0) {
				err = -1;
				error("%ldth package send timeout\n", i);
				break;
			}
		}
		timer_stop(&timer);
		if (err)
			return -1;
		info("xmodem upgrading %ld%%\r", (i * 100 / total));
	}
