...
static void system_gpio_init(void)
{
	...
	pins = GPIO_PIN_6 | GPIO_PIN_8 | GPIO_PIN_9 | GPIO_PIN_12 ;
	gpio_bit_reset(GPIOC, pins);
	gpio_init(GPIOC, GPIO_MODE_OUT_PP, GPIO_OSPEED_2MHZ, pins);

	/* GPIOC INPUT PINS */
	pins = GPIO_PIN_13;
	gpio_init(GPIOC, GPIO_MODE_IN_FLOATING, GPIO_OSPEED_2MHZ, pins);

	/* GPIOD */
	/* GPIOD OUTPUT PINS*/
	pins = GPIO_PIN_9 | GPIO_PIN_10 | GPIO_PIN_11| GPIO_PIN_15;
