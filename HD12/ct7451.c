...
/*
 * channel 0: bm0 temperature
 * channel 1: bm1 temperature
 * */
static uint8_t tmp_channel[2] = {0, 1};
...
void ct7451_init(int idx)
{
	...
	if(ret)
		debug("ct7451%d_init enable extended mode fail!(read)\n",idx);
	tmp |= CT7451_RANGE_MASK;
	i2c_master_smbus_write_byte(I2C, CT7451_SLAVE_ADDR, SMBTO,
				    CT7451_CONFIG_WR_ADDR, tmp);
	if(ret)
		debug("ct7451%d_init enable extended mode fail!(write)\n",idx);
