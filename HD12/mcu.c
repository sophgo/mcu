...
static void mcu_match(void *priv, uint8_t addr, int dir)
{
	(void)addr;
...
void mcu_process(void)
{
	...
	case CMD_CHIP0_VDDR:
		...
		multiphase_set_out_voltage(0, 0, (int)temp);
		temp_data = multiphase_output_voltage(0, 0);
		vddr_volt[0][0] = (temp_data >> 8) & 0xff;
		vddr_volt[0][1] = temp_data & 0xff;
		break;
