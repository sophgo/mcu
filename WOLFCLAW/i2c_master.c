...
int i2c_master_smbus_write_block(int i2c, unsigned char addr,
			 unsigned long timeout, unsigned char length,
			 uint8_t *data)
{
	return i2c_transfer7_timeout(i2c, addr, timeout, (uint8_t *)data,
				 length, NULL, 0);
}

int i2c_master_smbus_read_block(int i2c, unsigned char addr,
			 unsigned long timeout, unsigned char cmd,
			 unsigned char length, uint8_t *data)
{
	return i2c_transfer7_timeout(i2c, addr, timeout, &cmd,
					1, (uint8_t *)data, length);
}
