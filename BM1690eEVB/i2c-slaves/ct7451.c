...
static uint8_t real_temp_to_reg(int temp)
{
	...
	if (is_ex_mode) {
		...
		reg = round(temp * 1.1841f - 15.194f + 3.0f + 64.0f);
	} else {
		if (temp < 0)
			reg = 0;
		else if (temp > 127)
			...
			reg = round(temp * 1.1841f - 15.194f + 3.0f);
	}
	return reg;
}
...
static struct i2c01_slave_op tmp451_slave = {
...
static void ct7451_update_temp(void)
{
	...
	set_soc_temp(round(ct7451_ctx.local_temp * 1.2058f - 31.819f));
...
static void ct7451_process(void)
{
	...
	if (local_temp > get_critical_temp()) {
		...
		if (ct7451_ctx.over_temp > CT7451_OVER_TEMP_MAX) {
			...
			power_off();
			ct7451_ctx.over_temp = 0;
			dbg_printf("Over Temperature, Board ");
			if (get_critical_action() == CRITICAL_ACTION_REBOOT) {
				set_needpoweron();
				dbg_printf("Rebooted\n");
			} else {
				dbg_printf("Powered OFF\n");
			}
