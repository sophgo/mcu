...
#define SG2042_TMP_OVER_POWER_OFF		1<<4
...
void mcu_a2acp_init(struct i2c01_slave_ctx *i2c_slave_ctx)
{
//	debug("[%3lu] %-25s :  %-20s :  %4d\n", chip_uptime(), __FUNCTION__, __FILE__, __LINE__);
	mcu_ctx.critical_action = CRITICAL_ACTION_REBOOT;
	mcu_ctx.critical_temp = 80;
...
{
	int V_NTC1 = adc_average_tab[0].value;
	int V_NTC2 = adc_average_tab[1].value;
	int V_I_12V_SYS = adc_average_tab[2].value;
	printf("%s = %d(mV)\n", "V_NTC1", V_NTC1);
	printf("%s = %d(mV)\n", "V_NTC2", V_NTC2);
	printf("%s = %d(mV)\n", "V_I_12V_SYS", V_I_12V_SYS);
