...
static void I2C_AddrMatch(I2C_HandleTypeDef *hi2c);
static void I2C_SlaveProcessDone(I2C_HandleTypeDef *hi2c, uint8_t addr);
static void I2C_EnableIRQ(I2C_HandleTypeDef *hi2c, uint16_t InterruptRequest);
static void I2C_DisableIRQ(I2C_HandleTypeDef *hi2c, uint16_t InterruptRequest);
static void I2C_FlushTXDR(I2C_HandleTypeDef *hi2c);
...
HAL_StatusTypeDef I2C_SlaveInit(I2C_HandleTypeDef *hi2c, uint8_t *pData,
		uint8_t size) {
	/* Process Locked */
	__HAL_LOCK(hi2c);
	...
	I2C_EnableIRQ(hi2c, I2C_XFER_RX_IT | I2C_XFER_TX_IT | I2C_XFER_LISTEN_IT);
...
HAL_StatusTypeDef I2C_SlaveAbort(I2C_HandleTypeDef *hi2c) {
	...
	I2C_DisableIRQ(hi2c, I2C_XFER_LISTEN_IT | I2C_XFER_TX_IT | I2C_XFER_RX_IT);
...
static uint8_t addr;
uint32_t ramsource;
uint32_t packet_length = 8;
uint8_t flash_write = 0;
static uint8_t cnt_update = 0;
static uint8_t package_cnt = 0;
uint8_t buf_update[FLASH_PAGE_SIZE + 4];
...
static HAL_StatusTypeDef I2C_SlaveISR(I2C_HandleTypeDef *hi2c,
	...
	if ((I2C_CHECK_FLAG(ITFlags, I2C_FLAG_AF) != RESET)
		...
		/* Read data from RXDR */
		buf = (uint8_t) hi2c->Instance->RXDR;
...
//			__HAL_I2C_GENERATE_NACK(hi2c);
		...
		if (buf == 0x42) {
			buf_update[cnt_update] = buf;
			cnt_update++;
		} else if ((buf == 0x4d) && (cnt_update == 1)) {
			buf_update[cnt_update] = buf;
			cnt_update++;
		} else if ((buf == 0x43) && (cnt_update == 2)){
			buf_update[cnt_update] = buf;
			cnt_update++;
		} else if ((buf == 0x55) && (cnt_update == 3)){
			buf_update[cnt_update] = buf;
			cnt_update++;
			update_flag = 1;
		} else if ((cnt_update > 3) && (cnt_update < 132)) {
			buf_update[cnt_update] = buf;
			cnt_update++;
		}

		if (cnt_update == 132) {
			STMFLASH_Write_Page(flashdestination + (package_cnt*128), buf_update + 4, 128);
			cnt_update = 0;
			update_flag = 0;
			package_cnt++;
		}
...
//	}
	...
	if (hi2c->State == HAL_I2C_STATE_BUSY_TX_LISTEN) {
		I2C_SlaveTxCallback(hi2c, addr);
	} else if (hi2c->State == HAL_I2C_STATE_BUSY_RX_LISTEN) {
		I2C_SlaveRxCallback(hi2c, addr);
	}
	hi2c->State = HAL_I2C_STATE_LISTEN;
...
__weak void I2C_AddrCallback(I2C_HandleTypeDef *hi2c) {
	UNUSED(hi2c);
}
__weak void I2C_SlaveTxCallback(I2C_HandleTypeDef *hi2c, uint8_t addr) {
	UNUSED(hi2c);
}
__weak void I2C_SlaveRxCallback(I2C_HandleTypeDef *hi2c, uint8_t addr) {
	UNUSED(hi2c);
//	ramsource = (uint32_t) & aPacketData[PACKET_DATA_INDEX];
//	ramsource = 8;
//	if (FLASH_If_Write(flashdestination, (uint32_t*) ramsource, packet_length/4) != FLASHIF_OK) {
//	//if (FLASH_If_Write(flashdestination, (uint32_t*) buf, packet_length/4) != FLASHIF_OK) {
//		while(1);
//	}
}
