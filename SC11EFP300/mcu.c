...
#define REG_BM1690_3_TMP	0x20
#define REG_BOARD_3_TMP		0x21
#define REG_BM1690_4_TMP	0x22
#define REG_BOARD_4_TMP		0x23
#define REG_VDDR_VOLT_2_L	0x24
#define REG_VDDR_VOLT_2_H	0x25
#define REG_VDDR_VOLT_3_L	0x26
#define REG_VDDR_VOLT_3_H	0x27
...
#define REG_DROOP_2_L		0x5c
#define REG_DROOP_2_H		0x5d
#define REG_DROOP_3_L		0x5e
#define REG_DROOP_3_H		0x5f
...
static uint8_t mcu_read(void *priv)
{
	...
	case REG_BOARD_2_TMP:
		...
		break;
	case REG_BM1690_3_TMP:
		ret = get_soc_temp(2);
		break;
	case REG_BOARD_3_TMP:
		ret = get_board_temp(2);
		break;
	case REG_BM1690_4_TMP:
		ret = get_soc_temp(3);
		break;
	case REG_BOARD_4_TMP:
		ret = get_board_temp(3);
		break;
	...
	case REG_VDDR_VOLT_1_H:
		ret = vddr_volt[1][0];
		break;
	case REG_VDDR_VOLT_2_L:
		ret = vddr_volt[2][1];
		break;
	case REG_VDDR_VOLT_2_H:
		ret = vddr_volt[2][0];
		break;
	case REG_VDDR_VOLT_3_L:
		ret = vddr_volt[3][1];
		break;
	case REG_VDDR_VOLT_3_H:
		ret = vddr_volt[3][0];
		break;
	case REG_I12V_L:
...
#define CMD_CHIP2_VDDR		0x03
#define CMD_CHIP3_VDDR		0x04
#define CMD_CHIP0_DROOP		0x11
#define CMD_CHIP1_DROOP		0x12
#define CMD_CHIP2_DROOP		0x13
#define CMD_CHIP3_DROOP		0x14
...
void mcu_process(void)
{
	...
	if (current_time - last_time_collect > COLLECT_INTERVAL) {
		...
		adc_data =  adc_read_sys_i12v();
		filter_in(&adc_averge_tab[1], adc_data);
		adc_data =  adc_read_pcie_i12v();
		filter_in(&adc_averge_tab[2], adc_data);
		adc_data =  adc_read_pcie_i3v3();
		filter_in(&adc_averge_tab[3], adc_data);
		temp_data = raa228234_output_voltage(0, 0);
		vddr_volt[0][0] = (temp_data >> 8) & 0xff;
		vddr_volt[0][1] = temp_data & 0xff;
		temp_data = raa228234_output_voltage(1, 0);
		vddr_volt[1][0] = (temp_data >> 8) & 0xff;
		vddr_volt[1][1] = temp_data & 0xff;
		temp_data = raa228234_output_voltage(2, 0);
		vddr_volt[0][0] = (temp_data >> 8) & 0xff;
		vddr_volt[0][1] = temp_data & 0xff;
		temp_data = raa228234_output_voltage(3, 0);
		vddr_volt[1][0] = (temp_data >> 8) & 0xff;
		vddr_volt[1][1] = temp_data & 0xff;
		temp_data = raa228234_out_droop(0, 0);
		droop_val[0][0] = (temp_data >> 8) & 0xff;
		droop_val[0][1] = temp_data & 0xff;
		temp_data = raa228234_out_droop(1, 0);
		droop_val[1][0] = (temp_data >> 8) & 0xff;
		droop_val[1][1] = temp_data & 0xff;
		temp_data = raa228234_out_droop(2, 0);
		droop_val[0][0] = (temp_data >> 8) & 0xff;
		droop_val[0][1] = temp_data & 0xff;
		temp_data = raa228234_out_droop(3, 0);
		droop_val[1][0] = (temp_data >> 8) & 0xff;
		droop_val[1][1] = temp_data & 0xff;
	...
	case CMD_CHIP0_VDDR:
		...
		raa228234_set_out_voltage(0, 0, (int)temp);
		temp_data = raa228234_output_voltage(0, 0);
		vddr_volt[0][0] = (temp_data >> 8) & 0xff;
		vddr_volt[0][1] = temp_data & 0xff;
		break;
	case CMD_CHIP1_VDDR:
		temp = byte2u16(set_vddr_val);
		raa228234_set_out_voltage(0, 1, (int)temp);
		temp_data = raa228234_output_voltage(0, 1);
		vddr_volt[0][0] = (temp_data >> 8) & 0xff;
		vddr_volt[0][1] = temp_data & 0xff;
		break;
	case CMD_CHIP2_VDDR:
		temp = byte2u16(set_vddr_val);
		raa228234_set_out_voltage(1, 0, (int)temp);
		temp_data = raa228234_output_voltage(1, 0);
		vddr_volt[0][0] = (temp_data >> 8) & 0xff;
		vddr_volt[0][1] = temp_data & 0xff;
		break;
	case CMD_CHIP3_VDDR:
		temp = byte2u16(set_vddr_val);
		raa228234_set_out_voltage(1, 1, (int)temp);
		temp_data = raa228234_output_voltage(1, 1);
		vddr_volt[0][0] = (temp_data >> 8) & 0xff;
		vddr_volt[0][1] = temp_data & 0xff;
		break;
	...
	case CMD_CHIP1_DROOP:
		...
		raa228234_set_out_voltage(0, 1, (int)temp);
		break;
	case CMD_CHIP2_DROOP:
		temp = byte2u16(set_droop_val);
		raa228234_set_out_voltage(1, 0, (int)temp);
		break;
	case CMD_CHIP3_DROOP:
		temp = byte2u16(set_droop_val);
		raa228234_set_out_voltage(1, 1, (int)temp);
		break;
	case CMD_UPDATE:
