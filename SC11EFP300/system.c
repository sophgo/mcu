...
static void system_gpio_init(void)
{
	...
	pins = GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_15;
	...
	/* -------------------------- GPIOB -------------------------- */
	/* GPIOB OUTPUT PINS */
	pins = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_8 | GPIO_PIN_9 |
	...
	pins = GPIO_PIN_5;
	gpio_mode_set(GPIOC, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, pins);
	...
	gpio_mode_set(GPIOD, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, pins);
	gpio_output_options_set(GPIOD, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, pins);
	gpio_bit_reset(GPIOD, pins);

	/* GPIOD INPUT PINS */
	pins = GPIO_PIN_11 | GPIO_PIN_12 | GPIO_PIN_13 | GPIO_PIN_15;
	gpio_mode_set(GPIOD, GPIO_MODE_INPUT, GPIO_PUPD_NONE, pins);

	/* -------------------------- GPIOE -------------------------- */
	/* GPIOE OUTPUT PINS */
	pins = GPIO_PIN_2;
	gpio_mode_set(GPIOE, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, pins);
	gpio_output_options_set(GPIOE, GPIO_OTYPE_PP, GPIO_OSPEED_2MHZ, pins);
	gpio_bit_set(GPIOE,pins);

	pins = GPIO_PIN_12 | GPIO_PIN_13;
	gpio_mode_set(GPIOE, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLDOWN, pins);
	gpio_output_options_set(GPIOE, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, pins);
	gpio_bit_reset(GPIOE,pins);

	/* GPIOE INPUT PINS */
	pins = GPIO_PIN_0 | GPIO_PIN_1 | GPIO_PIN_3 | GPIO_PIN_4 | GPIO_PIN_5 |
		GPIO_PIN_11 | GPIO_PIN_14 | GPIO_PIN_15;
	gpio_mode_set(GPIOE, GPIO_MODE_INPUT, GPIO_PUPD_NONE, pins);

	/* GPIOE OUTPUT WAKE PINS */
	pins = GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_8 | GPIO_PIN_9;
	gpio_mode_set(GPIOE, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLDOWN, pins);
	gpio_output_options_set(GPIOE, GPIO_OTYPE_PP, GPIO_OSPEED_2MHZ, pins);
	gpio_bit_reset(GPIOE,pins);
}
