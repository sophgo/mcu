...
void pin_init(void)
{
	...
	GPIO_OSPEEDR(GPIOA) =
	    (GPIO_OSPEEDR(GPIOA) & ~0xc3ff3fff) | (GPIO_OSPEED_LOW << (0 * 2)) |
	    (GPIO_OSPEED_LOW << (1 * 2)) | (GPIO_OSPEED_HIGH << (2 * 2)) |
	    (GPIO_OSPEED_HIGH << (3 * 2)) | (GPIO_OSPEED_LOW << (4 * 2)) |
	    (GPIO_OSPEED_LOW << (5 * 2)) | (GPIO_OSPEED_LOW << (6 * 2)) |
	    (GPIO_OSPEED_LOW << (8 * 2)) | (GPIO_OSPEED_HIGH << (9 * 2)) |
	    (GPIO_OSPEED_HIGH << (10 * 2)) | (GPIO_OSPEED_LOW << (11 * 2)) |
	...
	GPIO_MODER(GPIOA) =
	    (GPIO_MODER(GPIOA) & ~0xc3ff3fff) | (GPIO_MODE_ANALOG << (0 * 2)) |
	    (GPIO_MODE_ANALOG << (1 * 2)) | (GPIO_MODE_AF << (2 * 2)) |
	    (GPIO_MODE_AF << (3 * 2)) | (GPIO_MODE_OUTPUT << (4 * 2)) |
	    (GPIO_MODE_OUTPUT << (5 * 2)) | (GPIO_MODE_OUTPUT << (6 * 2)) |
	    (GPIO_MODE_INPUT << (8 * 2)) | (GPIO_MODE_AF << (9 * 2)) |
	    (GPIO_MODE_AF << (10 * 2)) | (GPIO_MODE_OUTPUT << (11 * 2)) |
	    (GPIO_MODE_OUTPUT << (12 * 2)) | (GPIO_MODE_OUTPUT << (15 * 2));
	...
	GPIO_MODER(GPIOB) =
	    ...
	    (GPIO_MODE_INPUT << (13 * 2)) | (GPIO_MODE_OUTPUT << (14 * 2)) |
	...
	/* output speed, low, medium, high, very high */
	GPIO_OSPEEDR(GPIOC) =
	    (GPIO_OSPEEDR(GPIOC) & ~0xfc0fcfff) | (GPIO_OSPEED_HIGH << (0 * 2))
	...
	/* pull-up pull-down */
	GPIO_PUPDR(GPIOC) =
	    (GPIO_PUPDR(GPIOC) & ~0xfc0fcfff) | (GPIO_PUPD_NONE << (0 * 2)) |
	    (GPIO_PUPD_NONE << (1 * 2)) | (GPIO_PUPD_NONE << (2 * 2)) |
	    (GPIO_PUPD_NONE << (3 * 2)) | (GPIO_PUPD_NONE << (4 * 2)) |
	...
	/* mode, input, output, alternate function or analog */
	GPIO_MODER(GPIOC) =
	    (GPIO_MODER(GPIOC) & ~0xfc0fcfff) | (GPIO_MODE_AF << (0 * 2)) |
	    (GPIO_MODE_AF << (1 * 2)) | (GPIO_MODE_ANALOG << (2 * 2)) |
	    (GPIO_MODE_INPUT << (3 * 2)) | (GPIO_MODE_OUTPUT << (4 * 2)) |
	    (GPIO_MODE_ANALOG << (5 * 2)) | (GPIO_MODE_OUTPUT << (7 * 2)) |
	    (GPIO_MODE_INPUT << (8 * 2)) | (GPIO_MODE_OUTPUT << (9 * 2)) |
	    (GPIO_MODE_OUTPUT << (13 * 2)) | (GPIO_MODE_OUTPUT << (14 * 2)) |
	    (GPIO_MODE_OUTPUT << (15 * 2));

	/* port D configuration */
	/* output data, default output state */
	GPIO_ODR(GPIOD) = INT_PMIC_B1234_PIN;
	/* output type, push-pull or open-drain */
	GPIO_OTYPER(GPIOD) = 0;
	/* output speed, low, medium, high, very high */
	GPIO_OSPEEDR(GPIOD) =
	    (GPIO_OSPEEDR(GPIOD) & ~0x3fc3ffc3) | (GPIO_OSPEED_LOW << (0 * 2)) |
	    (GPIO_OSPEED_LOW << (3 * 2)) | (GPIO_OSPEED_LOW << (4 * 2)) |
	    (GPIO_OSPEED_LOW << (5 * 2)) | (GPIO_OSPEED_LOW << (6 * 2)) |
	    (GPIO_OSPEED_LOW << (7 * 2)) | (GPIO_OSPEED_HIGH << (8 * 2)) |
	...
	/* pull-up pull-down */
	GPIO_PUPDR(GPIOD) =
	    (GPIO_PUPDR(GPIOD) & ~0x3fc3ffc3) | (GPIO_PUPD_NONE << (0 * 2)) |
	    (GPIO_PUPD_NONE << (3 * 2)) | (GPIO_PUPD_NONE << (4 * 2)) |
	    (GPIO_PUPD_NONE << (5 * 2)) | (GPIO_PUPD_NONE << (6 * 2)) |
	    (GPIO_PUPD_NONE << (7 * 2)) | (GPIO_PUPD_NONE << (8 * 2)) |
	    (GPIO_PUPD_NONE << (11 * 2)) | (GPIO_PUPD_NONE << (12 * 2)) |
	...
	/* mode, input, output, alternate function or analog */
	GPIO_MODER(GPIOD) =
	    (GPIO_MODER(GPIOD) & ~0x3fc3ffc3) | (GPIO_MODE_OUTPUT << (0 * 2)) |
	    (GPIO_MODE_INPUT << (3 * 2)) | (GPIO_MODE_INPUT << (4 * 2)) |
	    (GPIO_MODE_OUTPUT << (5 * 2)) | (GPIO_MODE_INPUT << (6 * 2)) |
	    (GPIO_MODE_OUTPUT << (7 * 2)) | (GPIO_MODE_INPUT << (8 * 2)) |
	    (GPIO_MODE_INPUT << (11 * 2)) | (GPIO_MODE_INPUT << (12 * 2)) |
	    (GPIO_MODE_INPUT << (13 * 2)) | (GPIO_MODE_INPUT << (14 * 2));
	...
	/* output speed, low, medium, high, very high */
	GPIO_OSPEEDR(GPIOE) =
	    (GPIO_OSPEEDR(GPIOE) & ~0xffcf3ccf) | (GPIO_OSPEED_LOW << (0 * 2)) |
	    (GPIO_OSPEED_LOW << (1 * 2)) | (GPIO_OSPEED_LOW << (3 * 2)) |
	    (GPIO_OSPEED_LOW << (5 * 2)) | (GPIO_OSPEED_LOW << (6 * 2)) |
	    (GPIO_OSPEED_HIGH << (8 * 2)) | (GPIO_OSPEED_HIGH << (9 * 2)) |
	    (GPIO_OSPEED_LOW << (11 * 2)) | (GPIO_OSPEED_LOW << (12 * 2)) |
	    (GPIO_OSPEED_LOW << (13 * 2)) | (GPIO_OSPEED_LOW << (14 * 2)) |
	    (GPIO_OSPEED_LOW << (15 * 2));
	/* pull-up pull-down */
	GPIO_PUPDR(GPIOE) =
	    (GPIO_PUPDR(GPIOE) & ~0xffcf3ccf) | (GPIO_PUPD_NONE << (0 * 2)) |
	    (GPIO_PUPD_NONE << (1 * 2)) | (GPIO_PUPD_NONE << (3 * 2)) |
	    (GPIO_PUPD_NONE << (5 * 2)) | (GPIO_PUPD_NONE << (6 * 2)) |
	    (GPIO_PUPD_NONE << (8 * 2)) | (GPIO_PUPD_NONE << (9 * 2)) |
	    (GPIO_PUPD_NONE << (11 * 2)) | (GPIO_PUPD_NONE << (12 * 2)) |
	    (GPIO_PUPD_NONE << (13 * 2)) | (GPIO_PUPD_NONE << (14 * 2)) |
	...
	/* mode, input, output, alternate function or analog */
	GPIO_MODER(GPIOE) =
	    (GPIO_MODER(GPIOE) & ~0xffcf3ccf) | (GPIO_MODE_OUTPUT << (0 * 2)) |
	    (GPIO_MODE_OUTPUT << (1 * 2)) | (GPIO_MODE_OUTPUT << (3 * 2)) |
	    (GPIO_MODE_OUTPUT << (5 * 2)) | (GPIO_MODE_OUTPUT << (6 * 2)) |
	    (GPIO_MODE_AF << (8 * 2)) | (GPIO_MODE_AF << (9 * 2)) |
	    (GPIO_MODE_INPUT << (11 * 2)) | (GPIO_MODE_INPUT << (12 * 2)) |
	    (GPIO_MODE_OUTPUT << (13 * 2)) | (GPIO_MODE_OUTPUT << (14 * 2)) |
